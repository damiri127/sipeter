...
@section('content')
    <div class="card">
        <div class="card-header">
            <h2>Tambah Data Program {{$sub_kategori_ukm->nama_sub_kategori_ukm}}</h2>
        </div>
        <form action="{{route('store_program_ukm', ['id_sub_kategori_ukm'=>$sub_kategori_ukm->id_sub_kategori_ukm])}}" method="POST">
            @csrf
            <div class="card-body">
                <div class="form-group">
                    <label for="inputNama">Nama Program</label>
                    <input id="inputNama" type="text" name="nama_program_ukm" class="form-control" required placeholder="Masukan Nama Sub Kategori UKM...">
                </div>
                <div class="form-group">
                    <label for="inputNama">Penanggung Jawab</label>
                    <input type="text" name="nama_penanggung_jawab_program_ukm" id="nama_pj" class="form-control" required placeholder="Masukan Nama Penanggung Jawab Sub Kategori UKM...">
                    <input required class="form-control" type="hidden" name="penanggung_jawab_program_ukm" id="id_pj_ukm">
                </div>
            </div>
            <div class="card-footer">
                <button class="btn btn-success" type="submit">Simpan</button>
                <a href="#" class="btn btn-danger" id="backConfirmation" data-href="{{route('mengelola_program_ukm', ['id_sub_kategori_ukm'=>$sub_kategori_ukm->id_sub_kategori_ukm])}}">Kembali</a>
